...
import { Gift, Phone, Key, User, Building, Users, TrendingUp } from 'lucide-react';
...
export function UnifiedAuth({ defaultRole = 'business_owner' }) {
  ...
  async function resolveDestination(fallback) {
    // اگر next وجود دارد، همیشه به همون صفحه برگرد - حتی اگر چند فضای کاری داری
    // چون کاربر از همون صفحه لاگین کرده (مثلاً هوم)
    const paramsNext = new URLSearchParams(window.location.search).get('next');
    const effectiveNext = nextPath || paramsNext;
    if (effectiveNext) {
      // اگر چند فضا داری و next یک داشبورد عمومی مثل /business است که مبهمه، ببر به انتخاب فضا ولی next رو نگه دار
      if (effectiveNext.startsWith('/business') || effectiveNext.startsWith('/staff') || effectiveNext.startsWith('/visitor') || effectiveNext.startsWith('/admin')) {
        try {
          ...
          if (wsData.ok && wsData.total > 1) {
            // اگر next دقیقاً /business یا /staff است و چند بیزنس داری، انتخاب فضا منطقی‌تره
    ...
    // اگر next نداشت (مثلاً از هوم اومده /login) → برو هوم، نه داشبورد
    return '/';
  }
  ...
  async function handleVerifyOtp(e) {
    ...
    try {
      ...
      // بعد از لاگین موفق: اگر next داشت برو همونجا، وگرنه برو هوم (نه داشبورد)
      let redirect = nextPath || '/';
      if (ref && !redirect.includes('ref') && redirect === '/') redirect = `/?ref=${ref}`;
    ...
    <div className="min-h-dvh flex items-center justify-center bg-gradient-to-br from-[#F0F9FF] via-white to-[#EFF7FB] px-4 py-12">
      <Card className="w-full max-w-md shadow-xl border border-[#E0F0F8] bg-white/80 backdrop-blur-xl">
        <CardHeader className="text-center pb-2">
          <div className="mx-auto size-14 rounded-2xl bg-primary text-white flex items-center justify-center font-black text-xl shadow-lg shadow-primary/20">ن</div>
          <CardTitle className="mt-5 text-[22px] font-black tracking-tight">
          ...
          <CardDescription className="leading-6 text-[13px]">
        ...
        <CardContent className="space-y-5 pt-2">
          {step === 'phone' && (
            <form onSubmit={handlePhoneSubmit} className="space-y-4" noValidate>
              <Input label="شماره موبایل" type="tel" inputMode="tel" dir="ltr" placeholder="0912..." value={phone} onChange={(e)=>setPhone(e.target.value)} required autoFocus autoComplete="tel" aria-describedby="phone-help" />
              <p id="phone-help" className="text-[11px] text-muted-foreground -mt-2">شماره ایرانی با 09 شروع می‌شود</p>
              <Button type="submit" className="w-full h-12 text-[15px] rounded-xl" loading={loading} aria-label="ادامه با پیامک">ادامه با پیامک ←</Button>
              <div className="relative py-2"><div className="absolute inset-0 top-1/2 h-px bg-border" /><span className="relative bg-white px-3 text-[11px] text-muted-foreground mx-auto block w-fit">یا</span></div>
              <Button type="button" variant="secondary" className="w-full h-11 rounded-xl" onClick={()=>setStep('password')}>ورود با رمز عبور</Button>
              {ref && <p className="text-[11px] text-center bg-amber-50 border border-amber-200 rounded-xl py-2 flex items-center justify-center gap-1.5"><Gift className="size-3.5" /> کد معرف: <span className="font-mono font-bold">{ref}</span></p>}
            </form>
          )}

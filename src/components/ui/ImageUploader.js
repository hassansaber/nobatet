...
export function ImageUploader({ label, value, onChange, type = 'general', accept = 'image/*,video/*', hint, preview = true }) {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      onChange?.(data.url);
      setProgress('✓ آپلود و بهینه‌سازی شد');
          ...
          {preview && value && (
            <div className="shrink-0">
              ...
              <div className="size-24 rounded-xl overflow-hidden border bg-white shadow-sm">
                {isVideo ? (
                  <video src={value} className="w-full h-full object-cover" controls muted loop />
                ) : (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {values.map((url, idx) => (
          <div key={idx} className="relative group rounded-xl overflow-hidden border aspect-[4/3] bg-slate-50">
            {url.match(/\.(mp4|webm)$/i) ? <video src={url} className="w-full h-full object-cover" controls /> : <img src={url} alt={`gallery ${idx}`} className="w-full h-full object-cover" />}

...
export async function generateMetadata({ params }) {
  ...
  // تک سطحی جدید: moristyle.nobatet.com - ابتدا بیزنس، سپس ویزیتور
  const biz = await getPublicBusinessLanding(slug).catch(() => null);
  if (biz) {
    return {
      title: biz?.name || slug,
  ...
  const v = await getVisitorBySlug(slug).catch(() => null);
  if (v) return { title: `ویزیتور ${v.name}` };
  return { title: slug };
}
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/50 glass-strong shadow-2xl">
              ...
              <div className="p-4 grid grid-cols-2 gap-2.5 text-[12px]">
                {[
                  { t: 'ساب‌دامین اختصاصی', d: 'moristyle.nobatet.com' },
...
function RenderBusiness({ biz, slug }) {
    ...
    <div className="min-h-dvh bg-slate-50 pb-28">
      ...
      <main className="mx-auto max-w-3xl px-4 py-5 sm:py-7 space-y-5">
          ...
          <section>
            ...
            <div className="rounded-xl glass overflow-hidden shadow-sm">
              ...
              <div className="relative aspect-[16/10] bg-slate-100">
                <iframe title="نقشه" className="absolute inset-0 h-full w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src={`https://maps.google.com/maps?q=${encodeURIComponent([biz.address, biz.city].filter(Boolean).join('، ') || biz.name)}&z=15&output=embed`} />
              </div>
              <div className="p-2.5 border-t border-white/40 flex justify-between items-center">
                <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([biz.address, biz.city].filter(Boolean).join('، ') || biz.name)}`} target="_blank" rel="noopener noreferrer" className="text-[11px] font-medium hover:underline inline-flex items-center gap-1" style={{ color: primary }}>
        ...
        <p className="text-center text-[10px] text-muted-foreground pt-1">
          قدرت‌گرفته از <a href="/" className="font-medium" style={{ color: primary }}>نوبتت</a> • رزرو هوشمند • {biz.slug}.nobatet.com
        </p>
      </main>
...
export default async function TenantLandingPage({ params }) {
  ...
  const [biz, visitor] = await Promise.all([
    getPublicBusinessLanding(slug).catch(() => null),
    getVisitorBySlug(slug).catch(() => null),
  ]);

  // اگر type صریحاً visitor است
  if (type === 'visitor') {
    if (visitor) return RenderVisitor({ visitor });
    if (biz) return RenderBusiness({ biz, slug }); // fallback برای تک سطحی
    notFound();
  }

  // type business یا auto (تک سطحی جدید)
  if (biz) return RenderBusiness({ biz, slug });
  if (visitor) return RenderVisitor({ visitor });
  notFound();
}
